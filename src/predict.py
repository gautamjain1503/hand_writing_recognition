...
def word_pred(img,cord,ctc_beam):
    ...
    for i in word:
        print("\n",i)
        print((img[i[1]:i[3],i[0]:i[2]]).shape)
...
def get_img_text(img):
    model = Model(char_list_from_file(), DecoderType.BestPath , must_restore=False, dump=False)
    shape_img=img.shape
    dim=shape_img[0:2]
